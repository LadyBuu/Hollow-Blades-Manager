...
function renderTeamList(teams, tab) {
    ...
    teams.forEach(function(team) {
        ...
        if (tab === 'temporary' && team.temporaryMission) {
            var mission = getMission(team.temporaryMission);
            if (mission) {
                missionDisplay = ' <span style="font-size:0.6rem;color:var(--text-dim);">(' + mission.title + ')</span>';
            }
        }
        
        html += '<div class="list-item team-item" data-id="' + team.id + '" style="grid-template-columns:1.5fr 0.8fr 0.8fr 0.8fr 1fr;">';
        html += '<span><strong>' + team.name + '</strong>' + missionDisplay + '</span>';
        
        if (tab === 'academic') {
            html += '<span>' + periodDisplay + '</span>';
...
function attachFilterEvents(tab) {
    ...
    if (filterWeek) {
        filterWeek.addEventListener('change', function() {
            teamManagerState.filterWeek = parseInt(this.value) || 1;
            renderTeamTab(tab);
        });
    }
