/**
 * class-view.js - Class View Module
...
function renderClassData() {
    ...
    if (disciplines.length === 0) {
        container.innerHTML = '<p class="empty-state">No ' + classViewState.filterDiscipline + ' disciplines available for week ' + classViewState.currentWeek + '</p>';
        return;
    }
    ...
    disciplines.forEach(function(discipline) {
        // Build class data grouped by instructor
            ...
            for (var day in schedule) {
                for (var hour in schedule[day]) {
                    if (String(schedule[day][hour]) === String(discipline.id)) {
                        // Get the instructor for this specific class
                        var instructorId = null;
                        if (typeof getClassInstructor === 'function') {
                            instructorId = getClassInstructor(student.id, classViewState.currentWeek, parseInt(day), parseInt(hour));
                        }
        ...
        // Only show disciplines that have classes
        ...
        // Sort instructor groups - assigned first, then unassigned
        ...
        instructorKeys.forEach(function(key) {
            ...
            if (group.instructorId) {
                var instructor = data.characters.find(function(c) { 
                    return String(c.id) === String(group.instructorId); 
                });
                if (instructor) {
            ...
            classKeys.forEach(function(classKey) {
                ...
                html += '<div class="class-time">' + dayNames[cls.day] + ' at ' + hourDisplay + ':00 ' + ampm + 
                    ' <span style="color:var(--text-dim);font-size:0.7rem;">(' + cls.students.length + ' student' + (cls.students.length > 1 ? 's' : '') + 
                    (isFull ? ' <span style="color:var(--danger);">FULL</span>' : '') + ')</span></div>';
                html += '<div>';
                cls.students.forEach(function(student) {
                    ...
                    html += '<span class="student-tag" style="' + (isDeceased ? 'opacity:0.4;text-decoration:line-through;' : '') + '">' + name + ' <span style="color:var(--text-dim);font-size:0.6rem;">(' + status + ')</span></span>';
                });
                html += '</div>';
                html += '</div>';
            });
...
function populateClassFilter() {
    ...
    } else if (classViewState.filterDiscipline) {
